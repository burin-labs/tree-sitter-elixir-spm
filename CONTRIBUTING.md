# Contributing to `tree-sitter-elixir-spm`

## Scope

This repository is a packaging wrapper, not a product. It exists so Swift
projects can depend on the upstream Elixir tree-sitter grammar without running
...
[`tree-sitter-elixir`](https://github.com/elixir-lang/tree-sitter-elixir) tree-sitter grammar. The generated parser
(`Sources/TreeSitterElixir/src/parser.c` and any `scanner.c`) is **vendored** so
...
You should see `T _tree_sitter_elixir`. If the grammar ships an
...
2. Copy `src/parser.c`, `src/scanner.c` (if present), and `src/tree_sitter/*.h`
   into `Sources/TreeSitterElixir/src/`.
